...
const initiatePayment = async (req, res, next) => {
  try {
    ...
    // Get vendor with eSewa credentials (select all defaults + hidden secretKey)
    ...
    if (!vendor?.payment?.esewa?.isEnabled || !vendor.payment.esewa.merchantCode) {
      return res.status(400).json({
        success: false,
        message: "Payment is not configured for this store.",
      });
    }
    ...
    // eSewa requires all amounts as strings — keep them consistent
    const amount = String(order.summary.total);
    const taxAmount = "0";
    const productServiceCharge = "0";
    const productDeliveryCharge = "0";
    const totalAmount = amount; // amount + tax + service + delivery (all zero extras)

    // Generate signature — message fields must match signed_field_names order exactly
    ...
    try {
      ...
      if (secretKey && decoded.signed_field_names && decoded.signature) {
        ...
        if (expectedSignature === decoded.signature) {
          ...
          console.warn("eSewa redirect signature mismatch — rejecting payment");
        }
      } else {
...
const getCustomerOrders = async (req, res, next) => {
  try {
    ...
    // Scope to current vendor if on a subdomain
    if (req.vendor) {
...
const updatePaymentSettings = async (req, res, next) => {
  try {
    const { merchantCode, secretKey, isEnabled } = req.body;
